...
class ChefMenuRemoteDataSourceImp extends BaseRemoteDataSourceImpl
  ...
  Future<List<ChefMealModel>> getChefCategoryMeals(
      {required String token, required int chefId, required int categoryId})  async
  // {
  ...
  =>
      await performGetListRequest<ChefMealModel>(
        endpoint: Endpoints.getChefCategoryMeals(chefId , categoryId,),
        token: token,
      );

  @override
  Future<List<SubscriptionModel>> getChefSubscriptions({required String token, required int chefId}) async
